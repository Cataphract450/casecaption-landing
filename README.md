...
```text
Документные сценарии из одного заполнения
```

Фокус лендинга:
- повторяемые документные сценарии, не одиночный генератор счета/акта;
- пользователь сам выбирает состав пакета документов;
- один intake переиспользуется в договоре, счете, акте, спецификации и других шаблонах;
- без публичных цен и коммерческой упаковки до отдельного решения;
- без обещаний заменить `1C`, ЭДО или бухгалтерию.
- клиентский сотрудник должен сам выпускать комплект через CaseCaption после настройки.
...
- Нет публичных цен и коммерческой упаковки.
